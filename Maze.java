...
import javax.swing.JPanel;     //  变量导入到全局作用域中
...
class Maze extends JPanel {    // 设置布局管理器
    ...
    private void init() {
        ...
        synchronized private void move(int c) {
            int tx = ballX, ty = ballY;      // System.out.println(c);
            switch (c) {
...
protected void paintComponent(Graphics g) {
    ...
    g.setColor(Color.RED);      //  设置小人的颜色
    g.fillOval(getCenterX(ballY) - width / 3, getCenterY(ballX) - width / 3, width / 2, width / 2);
    if (drawPath == true)
        drawPath(g);
    }
    private void drawPath(Graphics g) {       //  添加路径
        Color PATH_COLOR = Color.BLUE;      // 设置生成路线的颜色
        if (drawPath == true)
...
public static void main(String[] args) {
    ...
    JFrame frame = new JFrame("走迷宫(按空格键显示或隐藏路径)");
